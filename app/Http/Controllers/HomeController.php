    ...
    public function viewer(Request $request) {
        ...
        foreach ($uniqueProductNames as $item) {
            ...
            if ($product) {
                ...
                if (empty($imgPath)) {
                    $imgUrl = asset('img/product/default.png');
                }
            
                elseif (str_contains($imgPath, 'http') || str_contains($imgPath, 'img/product/')) {
                    $imgUrl = asset($imgPath);
                }
          
                else {
                    $imgUrl = asset('img/product/' . $imgPath);
                }
        ...
        return view('home', compact('exchangeData', 'uniqueProducts', 'products', 'blockchains', 'blockchainMap', 'autoFrom', 'autoTo', 'liveNames'));
    }
}